document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Opcional: Se o backend retornar um token, você pode salvá-lo.
                // if(data.token) {
                //     localStorage.setItem('gatonautaToken', data.token);
                // }

                // Redirecionar para uma página de perfil ou dashboard após o login
                // Se não houver uma página de "painel", pode redirecionar para a 'index.html'
    ...
    function showMessage(message, type) {
        if (authMessage) {
            ...
            // Garanta que a div de mensagem está visível
            // Se você tiver estilos CSS que a ocultam por padrão, pode precisar disto:
    ...
    function clearMessages() {
        if (authMessage) {
            ...
            authMessage.style.display = 'none'; // Oculta a mensagem
            authMessage.className = 'form-message'; // Reset classes
        }
    }
});